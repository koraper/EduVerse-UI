...
| 버전 | 일자 | 변경 내용 | 작성자 |
|------|------|----------|--------|
| 1.2 | 2025-10-28 | 인증 정책 v1.0 반영: 회원가입/로그인 UI 업데이트 (탭 제거, 비밀번호 정책 변경, 로그인 상태 유지 제거) | Claude AI |
...
4. [AUTH-03 이메일 인증 화면](#4-auth-03-이메일-인증-화면)
5. [반응형 레이아웃](#5-반응형-레이아웃)
6. [폼 검증 및 에러 처리](#6-폼-검증-및-에러-처리)
7. [컴포넌트 명세](#7-컴포넌트-명세)
...
│  │ 3  │ │ 8  │ │ 4  │ │ 7  │ │ 2  │ │ 9  │              │
│  └────┘ └────┘ └────┘ └────┘ └────┘ └────┘              │
│  (each: w-12, h-14, text-center, text-2xl)               │
│                                                            │
│  ⏱️ 남은 시간: 09:35                                       │
│  (text-gray-400)                                          │
│                                                            │
...
│ 3  │ │ 8  │ │ 4  │ │ 7  │ │ 2  │ │ 9  │
└────┘ └────┘ └────┘ └────┘ └────┘ └────┘
(border: border-red-500, shake animation)

❌ 인증번호가 일치하지 않습니다 (4회 남음)
(text-red-500)
```
...
2. 자동으로 "인증 완료" 버튼 활성화
   ↓
3. "인증 완료" 클릭 (또는 마지막 자리 입력 시 자동 제출)
   ↓
4. 버튼 상태 변경:
   [인증 완료] → [⏳ 인증 중...] (disabled, loading spinner)
   ↓
5. POST /api/auth/verify-email 호출
   ↓
6a. 성공:
   - ✅ 토스트: "이메일 인증이 완료되었습니다"
   - 회원가입 완료 처리
...
- [ ] 이메일 중복 확인이 작동하는가?
- [ ] 학생 회원가입 페이지 제목이 명확한가?
- [ ] 로그인 실패 시 적절한 오류가 표시되는가?
- [ ] 계정 잠금 시 타이머가 표시되는가?
- [ ] 모바일에서 레이아웃이 깨지지 않는가?
- [ ] "계정(이메일) 찾기"와 "비밀번호 찾기" 버튼이 작동하는가?
- [ ] "문의하기" 버튼이 랜딩 페이지로 연결되는가?
